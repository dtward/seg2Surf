...
function [F,V] = seg2Surf(varargin)
disp('inputs')
disp(varargin)
% keyboard
% test
...
%         smooth = 0.5;
        ...
        if draw
            subplot(2,2,1)
            patch('faces',f,'vertices',v,'facecolor','r','edgecolor','k','facealpha',1);
            view(30,30)
            axis image
            light
            title('Matlab marching cubes isosurface')
            subplot(2,2,3)
            patch('faces',f,'vertices',v,'facecolor','r','edgecolor','k','facealpha',1);
        end
...
%         OPT.verbose = 1;
        ...
        if draw
            ...
            axis image
            light
            title('Restricted Delaunay triangulation')
            subplot(2,2,3)
            patch('faces',F,'vertices',V,'facecolor','b','edgecolor','k','facealpha',1);
            view(30,30)
            axis image
        ...
        if length(labels) > segMax
            ...
            writebyu_local(F,V, [f '.byu'])
            break; % only once
        else
            disp(['Writing file ' f '_label' num2str(labels(l),'%02d') '.byu'])
            writebyu_local(F, V, [f '_label' num2str(labels(l),'%02d') '.byu']);
        end
        Fsave{l} = F;
        Vsave{l} = V;
    end
